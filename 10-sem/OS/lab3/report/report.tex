...
% Під блоками коду без префікса «Лістинг N:»; лишається лише текст caption={...}
\captionsetup[lstlisting]{labelformat=empty, labelsep=none}

% Таблиці за ДСТУ: назва зверху, «Таблиця N – …», без вертикальних ліній
...
\begin{table}[H]
\caption*{Основні параметри роботи}
\small
\dstutablerules
...
Модель процесів & два ОС-процеси (\texttt{process\_a}, \texttt{process\_b}) + супервізор з GUI \\
...
\subsection{Два ОС-процеси + супервізор}

Модель складається з трьох процесів: \texttt{process\_a} і
\texttt{process\_b} --- окремі ОС-процеси, що спільно користуються
сегментом System~V SHM; супервізор --- третій процес із графічним
інтерфейсом (Fyne), який створює сегмент, запускає робочі процеси й
збирає події з їхніх каналів виводу.
...
\begin{table}[H]
\caption{Поля структури \texttt{State} у спільній пам'яті}
\dstutablerules
...
супервізор запускається з \texttt{LAB3\_CAPTURE=1} у повноекранному
режимі (Fyne \texttt{SetFullScreen}), після чого \texttt{screencapture -R}
знімає прямокутник вікна за \texttt{position}/\texttt{size}. У PDF вони вставлені як
...
\texttt{time.Sleep}, які діють як часткові бар'єри. Разом із цим
варто врахувати таке:
\begin{itemize}
...
відірватися. Спостереження узгоджується з обмеженим очікуванням
(bounded waiting) для двох процесів.
...
\begin{enumerate}
    ...
    \texttt{runtime.Gosched}. Стандартне вирішення --- атомарні
    операції з seq-cst (як у нашій основній реалізації).

    \item \textbf{Окремі процеси та System V SHM} --- реалізація побудована на
    типовому міжпроцесному обміні: \texttt{shmget}/\texttt{shmat}/\texttt{shmctl(IPC\_RMID)},
    ...
    \begin{itemize}
        ...
        громіздким.
        \item Програє у ефективності використання процесора: семафор з
        блокуванням у ядрі не виконує активного циклу очікування.
    \end{itemize}
\end{enumerate}
...
\begin{table}[H]
\caption{Перелік основних файлів проєкту}
\footnotesize
\dstutablerules
